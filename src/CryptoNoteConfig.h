...
namespace CryptoNote
{
	namespace parameters
	{
		...
		const uint64_t CRYPTONOTE_BLOCK_FUTURE_TIME_LIMIT = 360;            /* Works with LWMA3 */
		const uint64_t CRYPTONOTE_DEFAULT_TX_SPENDABLE_AGE = 30;            /* ~1 hour */
		...
		const size_t CRYPTONOTE_MAX_TX_SIZE_LIMIT = CRYPTONOTE_BLOCK_GRANTED_FULL_REWARD_ZONE - CRYPTONOTE_COINBASE_BLOB_RESERVED_SIZE;
		const size_t CRYPTONOTE_OPTIMIZE_SIZE = 100;

		const uint64_t CRYPTONOTE_MEMPOOL_TX_LIVETIME = (60 * 60 * 12);
		const uint64_t CRYPTONOTE_MEMPOOL_TX_FROM_ALT_BLOCK_LIVETIME = (60 * 60 * 12);
